import { NextApiRequest, NextApiResponse } from "next"
import { Contact } from "../../../model"
import { ApiErrorResponse } from "../../../model/ApiErrorResponse"
import { testContacts } from "../../../model/TestData"

/**
 * CONTACT restful-api with path-parameter
 * @param req NextApiRequest
 * @param res NextApiResponse
 */
export default (req: NextApiRequest, res: NextApiResponse) => {
  const {
    query: { id },
    method,
    body,
  } = req

  try {
    res.setHeader("Content-Type", "application/json")

    // validation
    const idStr = String(id)
    if (!/^\d+$/.test(idStr)) {
      const error: ApiErrorResponse = {
        statusCode: 400,
        message: "Please enter the Contact-id as a number.",
      }
      res.status(400).json(error)
      return
    }

    // find data
    const contactId = Number(idStr)
    const currentContact = testContacts
      .filter((contact) => contact.id === contactId)
      .find((contact) => !!contact)
    if (!currentContact) {
      const error: ApiErrorResponse = {
        statusCode: 404,
        message: `contact ${id} is not found.`,
      }
      res.status(404).json(error)
      return
    }

    switch (method) {
      case "GET":
        res.status(200).json(currentContact)
        break
      case "PUT":
        const newContact: Contact = body
        newContact.firstName = newContact.firstName
        testContacts[contactId] = body
        res.status(200).json(newContact)
        break
      case "DELETE":
        const deleteTargetId = testContacts.findIndex((contact) => contact.id === contactId)
        testContacts.splice(deleteTargetId, 1)
        res.status(204).end()
        break
      default:
        res.setHeader("Allow", ["GET", "POST", "PUT", "DELETE"])
        res.status(405).end(`Method ${method} Not Allowed`)
        break
    }
  } catch (e) {
    const error: ApiErrorResponse = {
      statusCode: 500,
      message: `Internal server error id. ${e}`,
    }
    res.status(500).json(error)
  }
}
